 ...
 *
 * Aggregates third-party payment processing fees for executive
 * reporting.
 *
...
export async function getFees({ from, to, granularity = 'day' }) {
  ...
  const { rows: totalRows } = await db.query(
    `SELECT
       COALESCE(SUM(processing_fee), 0)::numeric AS total,
       COUNT(*)::int AS count
       FROM payments
  ...
  const { rows: seriesRows } = await db.query(
    `SELECT DATE_TRUNC($1, created_at) AS bucket,
            COALESCE(SUM(processing_fee), 0)::numeric AS amount,
            COUNT(*)::int AS count
       FROM payments
      WHERE status = 'SUCCEEDED'
