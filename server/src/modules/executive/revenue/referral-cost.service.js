/**
 * Referral Cost Service
 *
 * Aggregates referral reward liabilities for executive reporting.
 * Referral rewards are a platform cost funded from platform income.
 *
...
export async function getCost({ from, to, granularity = 'day' }) {
  ...
  const { rows: totalRows } = await db.query(
    `SELECT
       COALESCE(SUM(reward_amount), 0)::numeric AS total,
       COUNT(*)::int AS count
  ...
  const { rows: seriesRows } = await db.query(
    `SELECT DATE_TRUNC($1, created_at) AS bucket,
            COALESCE(SUM(reward_amount), 0)::numeric AS amount,
            COUNT(*)::int AS count
       FROM referral_rewards
