...
const ProviderGrowth = function ProviderGrowth() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/executive/growth/providers', {
        credentials: 'include',
      });
      ...
      <div className="mt-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Text color="muted" className="text-xs">
              Provider registrations, certifications, and growth trends
            </Text>
          </div>
        </div>
        ...
        {data?.monthly && data.monthly.length > 0 ? (
          <Card padding="lg" className="mt-4">
            <Heading level={3} size="text-base">
              Monthly New Providers
            </Heading>
            <div className="mt-4">
              <BarChart
                data={data.monthly}
                bars={[{ dataKey: 'count', name: 'New Providers', color: '#8b5cf6' }]}
                xKey="month"
                height={300}
              />
            </div>
