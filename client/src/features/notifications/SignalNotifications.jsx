...
const SignalNotifications = function SignalNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/notifications?category=signal', {
        credentials: 'include',
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Text color="muted" className="text-xs">
              New signals and updates from your providers
            </Text>
          </div>
        </div>
