...
const SecurityNotifications = function SecurityNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/notifications?category=security', {
        credentials: 'include',
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Text color="muted" className="text-xs">
              Sign-in alerts, password changes, and device activity
            </Text>
          </div>
        </div>
