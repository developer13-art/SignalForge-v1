...
const BrokerReferralLinks = function BrokerReferralLinks() {
  ...
  const fetchLinks = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/ib/links', { credentials: 'include' });
  ...
  const handleCreate = useCallback(async () => {
    try {
      const response = await fetch('/api/ib/links', {
        method: 'POST',
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const handleRemove = useCallback(async () => {
    ...
    try {
      await fetch(`/api/ib/links/${removing.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      setRemoving(null);
  ...
  return (
    <Container size="lg" className="py-6">
      ...
      <Card padding="lg" className="mt-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <Text color="muted" className="text-xs">
              Unique links for each broker with IB tracking enabled
            </Text>
          </div>
        </div>
        ...
        <div className="mt-6">
          {loading ? (
            ...
            <EmptyState
              icon={Link2}
              title="No broker links yet"
              description="Generate a broker referral link to start tracking IB signups."
              action={
      ...
      <ConfirmDialog
        ...
        variant="danger"
        title="Delete broker link"
        description="Are you sure you want to delete this broker referral link?"
        confirmLabel="Delete"
      />
    </Container>
  );
...
export default BrokerReferralLinks;